  ...
  <div>
    ...
    <div id="visualiserEvenement-modal" class="modal">
      <div class="modal__contentVisualiserEv">
        <div class="form">
          <div class="title">
            <span>ÉVÉNEMENT "{{ titreModal }}"</span>
          </div>
  ...
  methods: {
    ...
    rejoindreEvenement() {
      ...
      if (this.res == true) {
        ...
        axios
          .put(
            "http://localhost:8080/evenements/" + this.idEvenement + "/rejoindre",
            {
              nom: this.$store.state.membre.utilisateur.nom+' '+this.$store.state.membre.utilisateur.prenom,
              status: "2",
              message: 'Événement : '+this.titreModal+' Date :'+this.dateModal,
            },
            config
          )
          .then((response) => {
            if (this.status == 2) {
              this.$router.push("/acepteInvitationAuth");
            } else {
          ...
          .catch((error) => {
            console.log("Error ========>", error);
            alert("C'est pas possible une autre fois, vous y êtes déjà attaché.");
          });
    },
